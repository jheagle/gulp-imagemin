...
const escapeMember = char => CLASS_MEMBERS_TO_ESCAPE.indexOf(char) < 0 ? char : ESCAPE + char;
...
const PLACEHOLDER = '\u0000';
const REGEX_RESTORE_PLACEHOLDER = new RegExp(`${PLACEHOLDER}(\\d+)${PLACEHOLDER}`, 'g');
...
const pinWildcards = source => {
  ...
  while (index < length) {
    ...
    if (source.startsWith(WILDCARD, index)) {
      ...
      tokens.push({
        boundary: source.slice(index, end)
      });
      index = end;
    } else if (char === '^' || char === '$') {
      tokens.push({
...
const basenameOf = path => {
  ...
  const index = path.lastIndexOf(SLASH, path[end] === SLASH ? end - 1 : end);
...
const parentOf = path => {
  ...
  const cut = path.lastIndexOf(SLASH, path.charCodeAt(end) === SLASH_CODE ? end - 1 : end);
...
class IgnoreRule {
  ...
  _make(mode, key) {
    const str = pinWildcards(this.regexPrefix.replace(REGEX_REPLACE_TRAILING_WILDCARD,
