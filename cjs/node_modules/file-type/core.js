...
exports.supportedMimeTypes = exports.supportedExtensions = void 0;
...
var Token = _interopRequireWildcard(require("../token-types/lib/index.js"));
var strtok3 = _interopRequireWildcard(require("../strtok3/lib/core.js"));
var _util = require("./util.js");
var _supported = require("./supported.js");
...
class FileTypeParser {
  ...
  async toDetectionStream(readableStream, options = {}) {
    const {
      default: stream
    } = await Promise.resolve().then(() => _interopRequireWildcard(require('node:stream')));
    const {
      ...
      readableStream.once('readable', () => {
        (async () => {
          try {
            ...
            const chunk = readableStream.read(sampleSize) ?? readableStream.read() ?? _nodeBuffer.Buffer.alloc(0);
            try {
  ...
  checkString(header, options) {
    return this.check((0, _util.stringToBytes)(header), options);
  }
  async parse(tokenizer) {
    this.buffer = _nodeBuffer.Buffer.alloc(minimumBytes);
    ...
    if (this.checkString('ID3')) {
      ...
      const id3HeaderLength = await tokenizer.readToken(_util.uint32SyncSafeToken);
    ...
    if (this.checkString('OggS')) {
      ...
      const type = _nodeBuffer.Buffer.alloc(8);
    ...
    if (this.checkString('%PDF')) {
      try {
        ...
        const buffer = _nodeBuffer.Buffer.alloc(Math.min(maxBufferSize, tokenizer.fileInfo.size));
        await tokenizer.readBuffer(buffer, {
          mayBeLess: true
        });

        // Check if this is an Adobe Illustrator file
        if (buffer.includes(_nodeBuffer.Buffer.from('AIPrivateData'))) {
          return {
            ext: 'ai',
            mime: 'application/postscript'
          };
    ...
    if (this.check([0x1A, 0x45, 0xDF, 0xA3])) {
      ...
      async function readField() {
        ...
        const id = _nodeBuffer.Buffer.alloc(ic + 1);
    ...
    if (this.check([0x30, 0x26, 0xB2, 0x75, 0x8E, 0x66, 0xCF, 0x11, 0xA6, 0xD9])) {
      async function readHeader() {
        const guid = _nodeBuffer.Buffer.alloc(16);
      ...
      while (tokenizer.position + 24 < tokenizer.fileInfo.size) {
        ...
        if (_check(header.id, [0x91, 0x07, 0xDC, 0xB7, 0xB7, 0xA9, 0xCF, 0x11, 0x8E, 0xE6, 0x00, 0xC0, 0x0C, 0x20, 0x53, 0x65])) {
          ...
          const typeId = _nodeBuffer.Buffer.alloc(16);
    ...
    if ((0, _util.tarHeaderChecksumMatches)(this.buffer)) {
      return {
        ext: 'tar',
...
const supportedExtensions = exports.supportedExtensions = new Set(_supported.extensions);
const supportedMimeTypes = exports.supportedMimeTypes = new Set(_supported.mimeTypes);