...
function decodeUTF8(bytes) {
  ...
  while (i < bytes.length) {
    ...
    if (b1 <= 0xef) {
      ...
      const b2 = bytes[i + 1];
      const b3 = bytes[i + 2];
      const valid = (b2 & 0xc0) === 0x80 && (b3 & 0xc0) === 0x80 && !(b1 === 0xe0 && b2 < 0xa0) &&
      // overlong
    ...
    const b3 = bytes[i + 2];
    const b4 = bytes[i + 3];
    const valid = (b2 & 0xc0) === 0x80 && (b3 & 0xc0) === 0x80 && (b4 & 0xc0) === 0x80 && !(b1 === 0xf0 && b2 < 0x90) &&
    // overlong
...
function encodeWindows1252(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (code >= 0x00 && code <= 0x7f || code >= 0xa0 && code <= 0xff) {
      out[i] = code;
      continue;
    }
    out[i] = 0x3f; // '?'
