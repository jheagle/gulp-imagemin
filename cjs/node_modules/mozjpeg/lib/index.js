...
const binWrapper = new _binWrapper.default().src(`${url}macos/cjpeg`, 'darwin').src(`${url}linux/cjpeg`, 'linux').src(`${url}win/cjpeg.exe`, 'win32').dest((0, _nodeUrl.fileURLToPath)(new URL('../vendor', require('url').pathToFileURL(__filename).toString()))).use(_nodeProcess.default.platform === 'win32' ? 'cjpeg.exe' : 'cjpeg');
