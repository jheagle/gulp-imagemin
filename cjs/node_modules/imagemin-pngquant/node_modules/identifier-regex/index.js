"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.default = identifierRegex;
var _index = _interopRequireDefault(require("../reserved-identifiers/index.js"));
function _interopRequireDefault(e) { return e && e.__esModule ? e : { default: e }; }
const baseRegex = /[$_\p{ID_Start}][$_\u200C\u200D\p{ID_Continue}]*/u;
const basePattern = `(?!${[...(0, _index.default)({
  includeGlobalProperties: true
})].join('|')})${baseRegex.source}`;
const regex = new RegExp(basePattern, 'u');
const regexExact = new RegExp(`^${basePattern}$`, 'u');
function identifierRegex({
  exact = true
} = {}) {
  return exact ? regexExact : regex;
}