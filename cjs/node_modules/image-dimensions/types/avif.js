...
exports.default = avif;
var _utilities = require("../utilities.js");
// Specification: https://aomediacodec.github.io/av1-avif/v1.1.0.html

const isAvif = bytes =>
// `ftypavif` magic bytes
bytes[4] === 0x66 && bytes[5] === 0x74 && bytes[6] === 0x79 && bytes[7] === 0x70 && bytes[8] === 0x61 && bytes[9] === 0x76 && bytes[10] === 0x69 && bytes[11] === 0x66;
function avif(bytes) {
  if (!isAvif(bytes)) {
    return;
  }
...
function unbox(data, offset) {
  ...
  const size = (0, _utilities.getUint32)(dataView, offset);
  if (size === undefined) {
    return;
  }
...
function parseIpcoBox(data, sizes) {
  ...
  while (offset < data.length) {
    ...
    if (box.type === 'ispe') {
      ...
      const width = (0, _utilities.getUint32)(dataView, 4);
      const height = (0, _utilities.getUint32)(dataView, 8);
