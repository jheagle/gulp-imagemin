...
Object.defineProperty(exports, "isIgnoredByIgnoreFiles", {
  enumerable: true,
  get: function () {
    return _ignore.isIgnoredByIgnoreFiles;
  }
});
Object.defineProperty(exports, "isIgnoredByIgnoreFilesSync", {
  enumerable: true,
  get: function () {
    return _ignore.isIgnoredByIgnoreFilesSync;
  }
});
...
var _index2 = _interopRequireDefault(require("../fast-glob/out/index.js"));
var _index3 = require("../path-type/index.js");
...
const directoryToGlob = async (directoryPaths, {
  ...
  const globs = await Promise.all(directoryPaths.map(async directoryPath => (await (0, _index3.isDirectory)(normalizePathForDirectoryGlob(directoryPath, cwd))) ? getDirectoryGlob({
    directoryPath,
    files,
    extensions
  }) : directoryPath));
...
const directoryToGlobSync = (directoryPaths, {
  ...
  files,
  extensions
} = {}) => directoryPaths.flatMap(directoryPath => (0, _index3.isDirectorySync)(normalizePathForDirectoryGlob(directoryPath, cwd)) ? getDirectoryGlob({
  directoryPath,
  files,
  extensions
}) : directoryPath);
...
const globby = exports.globby = normalizeArguments(async (patterns, options) => {
  ...
  const results = await Promise.all(tasks.map(task => (0, _index2.default)(task.patterns, task.options)));
  return unionFastGlobResults(results, filter);
});
const globbySync = exports.globbySync = normalizeArgumentsSync((patterns, options) => {
  const tasks = generateTasksSync(patterns, options);
  const filter = getFilterSync(options);
  const results = tasks.map(task => _index2.default.sync(task.patterns, task.options));
  return unionFastGlobResults(results, filter);
});
const globbyStream = exports.globbyStream = normalizeArgumentsSync((patterns, options) => {
  const tasks = generateTasksSync(patterns, options);
  const filter = getFilterSync(options);
  const streams = tasks.map(task => _index2.default.stream(task.patterns, task.options));
...
const isDynamicPattern = exports.isDynamicPattern = normalizeArgumentsSync((patterns, options) => patterns.some(pattern => _index2.default.isDynamicPattern(pattern, options)));
